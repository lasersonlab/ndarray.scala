// ~/sw/spark-2.2.1-bin-hadoop2.7/bin/spark-shell --master local[1] --driver-memory 8G --jars ~/.m2/repository/com/tom_e_white/hdf5-java-cloud/0.0.1-SNAPSHOT/hdf5-java-cloud-0.0.1-SNAPSHOT.jar
...
val file = "files/mca.loom" // change to "gs://..." for GCS
...
val rows = shardIndexes.flatMap(readShard(_)).map {case vec => Row(vec.toArray)}
