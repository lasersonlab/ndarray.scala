...
class LoadTest
  ...
  test("load") {
    ...
    implicit def cmpStructEntry[T]: Cmp[StructEntry[T]] = ???
    implicit def cmpStruct[S]: Cmp[Struct[S]] = ???
    implicit val cmpstruct: Cmp[struct] = ???
    ...
    !![Cmp[StructEntry[_]]]
    !![Cmp[DataType.Struct[Var]]]
//    !![Cmp[Seq[((Int, DataType), String)]]]
//
